...
private:
    ...
    static std::string c_type_name(const std::string& caret_name) {
        if (caret_name == "i8")   return "int8_t";
        if (caret_name == "i16")  return "int16_t";
        if (caret_name == "i32")  return "int32_t";
        if (caret_name == "i64")  return "int64_t";
        if (caret_name == "i128") return "__int128";
        if (caret_name == "isize")return "intptr_t";
        if (caret_name == "u8")   return "uint8_t";
        if (caret_name == "u16")  return "uint16_t";
        if (caret_name == "u32")  return "uint32_t";
        if (caret_name == "u64")  return "uint64_t";
        if (caret_name == "u128") return "unsigned __int128";
        if (caret_name == "usize")return "size_t";
        if (caret_name == "f32")  return "float";
        if (caret_name == "f64")  return "double";
        if (caret_name == "bool") return "bool";
        if (caret_name == "char") return "ccaret_char";
        if (caret_name == "void") return "void";
        if (caret_name == "string") return "ccaret_string";
        if (caret_name == "auto") return "int";
    ...
    void emit_function_decl(std::ostream& out, const ast::Decl& d) {
        ...
        if (is_void_type(d.fn_return_type)) ret = "void";
        out << ret << " " << d.fn_name << "(";
    ...
    void emit_stmt(std::ostream& out, const ast::StmtPtr& s, int indent = 1) {
        ...
        case ast::StmtKind::Block: {
            out << pad << "{\n";
            for (const auto& sub : s->block_stmts) emit_stmt(out, sub, indent + 1);
            out << pad << "}\n";
            break;
        }
        case ast::StmtKind::VarDecl: {
            std::string ty = emit_type(s->var_type);
            if (s->is_const) out << pad << "const ";
            else out << pad;
            out << ty << " " << s->var_name;
            if (s->var_init) {
                out << " = ";
        ...
        case ast::StmtKind::For:
            ...
            out << "; ";
            if (s->for_cond) emit_expr(out, s->for_cond);
            out << "; ";
            if (s->for_step) emit_expr(out, s->for_step);
            out << ") ";
    ...
    void emit_expr(std::ostream& out, const ast::ExprPtr& e) {
        ...
        case ast::ExprKind::CharLiteral:
            out << "(ccaret_char)'";
            if (e->char_value == '\n') out << "\\n";
            else if (e->char_value == '\t') out << "\\t";
            else if (e->char_value == '\\') out << "\\\\";
            else if (e->char_value == '\'') out << "\\'";
            else if (e->char_value == 0) out << "\\0";
            else out << static_cast<char>(e->char_value);
            out << "'";
            break;
        ...
        case ast::ExprKind::Unary: {
            ...
            case ast::UnaryOp::Plus:   op = "+"; break;
            case ast::UnaryOp::Not:    op = "!"; break;
            case ast::UnaryOp::BitNot: op = "~"; break;
            case ast::UnaryOp::PreInc: op = "++"; break;
            case ast::UnaryOp::PreDec: op = "--"; break;
            case ast::UnaryOp::PostInc:op = "++"; post = true; break;
        ...
        case ast::ExprKind::Binary: {
            ...
            case ast::BinaryOp::Mul: op = "*"; break;
            case ast::BinaryOp::Div: op = "/"; break;
            case ast::BinaryOp::Mod: op = "%"; break;
            case ast::BinaryOp::Eq:  op = "=="; break;
            case ast::BinaryOp::NotEq:op = "!="; break;
            case ast::BinaryOp::Lt:  op = "<"; break;
            case ast::BinaryOp::LtEq:op = "<="; break;
            case ast::BinaryOp::Gt:  op = ">"; break;
            case ast::BinaryOp::GtEq:op = ">="; break;
            case ast::BinaryOp::BitAnd:op = "&"; break;
            case ast::BinaryOp::BitOr: op = "|"; break;
            case ast::BinaryOp::BitXor:op = "^"; break;
            case ast::BinaryOp::Shl: op = "<<"; break;
            case ast::BinaryOp::Shr: op = ">>"; break;
