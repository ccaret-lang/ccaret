...
const char* sev_color(Severity s, bool color) {
    ...
    case Severity::Warning: return kBold;  // bold yellow applied below
    case Severity::Error:   return kBold;  // bold red applied below
    case Severity::Fatal:   return kBold;
    }
    return "";
}
const char* sev_color2(Severity s, bool color) {
    if (!color) return "";
    switch (s) {
    case Severity::Note:    return kReset;
...
std::string get_line(const std::string& src, std::uint32_t line) {
    ...
    if (cur == line && start < src.size()) return src.substr(start);
    return {};
}
...
std::string pad_left(std::uint32_t n, std::size_t w) {
    ...
    if (str.size() < w) str = std::string(w - str.size(), ' ') + str;
    return str;
}
}  // namespace
...
std::size_t DiagnosticsEngine::error_count() const {
    std::size_t n = 0;
    for (const auto& d : items_) {
        if (d.severity == Severity::Error || d.severity == Severity::Fatal) ++n;
    }
    return n;
}
...
void DiagnosticsEngine::render(std::ostream& os, const Diagnostic& d,
    ...
    const std::string loc  = d.file + ":" + std::to_string(d.line) + ":" +
    ...
    if (left.size() + 2 + loc.size() > target) {
        ...
        os << col1 << head << reset << " " << dim << ".." << reset << " "
           << body << "  " << dim << "··" << reset << " " << cyan << loc
           << reset << "\n";
    } else {
    ...
    if (!source_text.empty() && d.line > 0) {
        ...
        std::size_t span = d.evidence.empty() ? 1 :
        ...
        os << dim << num << reset << " " << dim << "|" << reset << " "
           << line_text << "\n";
        std::string gutter = "   " + dim + "|" + reset + " ";
        os << gutter;
        if (col0 > line_text.size()) col0 = line_text.size();
        os << std::string(col0, ' ') << caret_color << carets << reset;
        if (!d.hint.empty()) {
            const char* hint_col = d.hint_kind == SuggestionKind::Tip
                                       ? (color ? kBlue : "")
                                       : (color ? kGreen : "");
            const std::string label = d.hint_kind == SuggestionKind::Tip
                                          ? "tip:" : "try:";
