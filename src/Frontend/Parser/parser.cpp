...
private:
  ...
  ast::DeclPtr parse_top_level() {
    ...
    // Heuristic: if a Type followed by an identifier is a top-level var/const
    // or fn. We try var/const first, then fall through to function parsing.
    if (looks_like_top_var())
      return parse_top_var();
  ...
  bool looks_like_top_var() {
    ...
    if (p < toks_.size() && toks_[p].kind == TokenKind::KwMut)
      ++p;
    // skip * / & chain
    while (p < toks_.size() && (toks_[p].kind == TokenKind::Star ||
                                toks_[p].kind == TokenKind::Amp)) {
      ++p;
      if (p < toks_.size() && toks_[p].kind == TokenKind::KwMut)
        ++p;
    }
    // optional const/volatile after the pointer chain
    if (p < toks_.size() && (toks_[p].kind == TokenKind::KwConst ||
                             toks_[p].kind == TokenKind::KwVolatile))
      ++p;
    // expect identifier
    if (p >= toks_.size() || toks_[p].kind != TokenKind::Identifier)
      return false;
    ++p;
    if (p < toks_.size() && toks_[p].kind == TokenKind::KwMut)
      ++p;
    ...
    if (p >= toks_.size())
      return false;
    if (toks_[p].kind == TokenKind::Assign)
      return true;
    if (toks_[p].kind == TokenKind::Semicolon)
      return true;
    return false;
  }
  ...
  ast::StmtPtr parse_block() {
    ...
    while (!check(TokenKind::RBrace) && !at_eof()) {
      if (auto s = parse_stmt())
        blk->block_stmts.push_back(s);
      else if (!sync_recovery())
        break;
    }
    expect(TokenKind::RBrace, "`}`");
    return blk;
  ...
  ast::StmtPtr parse_stmt() {
    ...
    if (check(TokenKind::Semicolon)) {
      advance();
      return ast::make_stmt(ast::StmtKind::Empty, span);
    }
    if (check(TokenKind::LBrace))
      return parse_block();
    ...
    if (check(TokenKind::KwBreak)) {
      ...
      return ast::make_stmt(ast::StmtKind::Break, span);
    }
    if (check(TokenKind::KwContinue)) {
      advance();
      expect(TokenKind::Semicolon, "`;`");
      return ast::make_stmt(ast::StmtKind::Continue, span);
    }
    if (check(TokenKind::KwDefer))
      return parse_defer();
    if (is_type_start(cur().kind) && looks_like_var_decl())
  ...
  ast::StmtPtr parse_for() {
    ...
    if (!check(TokenKind::Semicolon))
      s->for_init = parse_stmt();
    if (!check(TokenKind::Semicolon))
      s->for_cond = parse_expr();
    expect(TokenKind::Semicolon, "`;`");
    if (!check(TokenKind::LBrace))
      s->for_step = parse_expr();
  ...
  ast::ExprPtr parse_assign() {
    ...
    if (check(TokenKind::Assign) || check(TokenKind::PlusAssign) ||
        ...
        check(TokenKind::AmpAssign) || check(TokenKind::PipeAssign) ||
        check(TokenKind::CaretAssign) || check(TokenKind::LtLtAssign) ||
        check(TokenKind::GtGtAssign)) {
  ...
  ast::ExprPtr parse_equality() {
    ...
    while (check(TokenKind::Eq) || check(TokenKind::NotEq)) {
      ast::BinaryOp op =
          check(TokenKind::Eq) ? ast::BinaryOp::Eq : ast::BinaryOp::NotEq;
      advance();
  ...
  ast::ExprPtr parse_shift() {
    ...
    while (check(TokenKind::LtLt) || check(TokenKind::GtGt)) {
      ast::BinaryOp op =
          check(TokenKind::LtLt) ? ast::BinaryOp::Shl : ast::BinaryOp::Shr;
      advance();
  ...
  ast::ExprPtr parse_additive() {
    ...
    while (check(TokenKind::Plus) || check(TokenKind::Minus)) {
      ast::BinaryOp op =
          check(TokenKind::Plus) ? ast::BinaryOp::Add : ast::BinaryOp::Sub;
      advance();
  ...
  static long long parse_int_literal(const std::string &s) {
    ...
    // strip suffix
    if (!t.empty() && (t.back() == 'u' || t.back() == 'i' || t.back() == 'l' ||
                       t.back() == 'f' || t.back() == 'd')) {
      t.pop_back();
    }
